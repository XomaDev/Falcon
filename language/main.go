...
func designTest() {
	...
	schemaString, err := design2.NewXmlParser(xmlString).ConvertXmlToSchema()
	if err != nil {
		panic(err)
	}
	println(schemaString)
	xmlString, err = design2.NewSchemaParser(schemaString).ConvertSchemaToXml()
	if err != nil {
		panic(err)
	}
...
func xmlTest() {
	...
	xmlString := string(codeBytes)
	exprs := analysis2.NewXMLParser(xmlString).ParseBlockly()
