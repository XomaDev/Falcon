...
func MakeSyntaxDiff(humanSyntax string, machineSyntax string) *SyntaxDiff {
	humanWords := makeSimpleWords(humanSyntax, lex2.NewLexer(&context.CodeContext{SourceCode: &humanSyntax, FileName: "human.diff"}).Lex())
	machineWords := makeSimpleWords(machineSyntax, lex2.NewLexer(&context.CodeContext{SourceCode: &machineSyntax, FileName: "machine.diff"}).Lex())
...
func makeSimpleWords(sourceCode string, tokens []*lex2.Token) []string {
	var words []string
